...
- (검증 완료) 헤더 정규식 `^([^kK]|[kK][^oO]).*`("ko 아님")이 Vercel 프리뷰에서 동작 확인됨. Vercel `has.value`가 헤더 값 전체를 매칭하므로 `.*`로 나머지를 소비해야 함(프리뷰에서 발견·수정). en/fr/ja→307, ko→200, 쿠키 오버라이드·에셋 200 전부 통과.
