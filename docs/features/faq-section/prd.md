...
- FAQPage 구조화 데이터를 추가해 SERP 리치 스니펫 노출을 확보한다.
...
2. 궁금한 질문을 클릭하면 답변이 아코디언으로 펼쳐진다.
3. 여러 항목을 동시에 펼쳐서 비교할 수 있다.
4. Google에서 "BugShot 무료인가요" 같은 검색 시 FAQ 리치 스니펫이 노출된다.

### FAQ 항목 (6개)

| # | 질문 (ko) | 요약 |
|---|-----------|------|
| 1 | BugShot은 무엇인가요? | 제품 개요 — 브라우저 안 올인원 버그 리포팅 |
| 2 | 어떤 브라우저에서 사용할 수 있나요? | Chrome + Chromium 기반(Edge, Brave, Arc) |
| 3 | 무료인가요? | 무료, 가입 불필요 |
| 4 | AI 리포트는 어떻게 생성되나요? | 수집 데이터 → AI 분석 → 구조화된 리포트 |
| 5 | 어떤 이슈 트래커를 지원하나요? | Jira, GitHub, Linear, Notion + Markdown 내보내기 |
| 6 | 수집된 데이터는 어떻게 처리되나요? | 로컬 처리, 외부 저장 없음 |

## 성공 기준

- [ ] FAQ 아코디언이 HowItWorks 아래, BottomCta 위에 렌더링된다.
- [ ] 6개 항목 모두 ko/en에서 펼침/접기 동작한다.
- [ ] 여러 항목 동시 펼치기가 가능하다.
